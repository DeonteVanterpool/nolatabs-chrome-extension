...
import {Context} from 'src/entries/background';
...
export async function handleCommit(args: string[], ctx: Context): Promise<Commit> {
    repoAddr: string, author: string, message: string, tabs: Tab[], branchNames: string[]
    if (args.length < 1) {
        throw new Error("Not enough arguments provided for commit command. Expected at least 4 arguments: repo address, author, message, and tabs. Optionally, branch names can be provided as additional arguments.");
    }
...
export async function getSnapshotForCommit(storage: chrome.storage.StorageArea, repo: Repository, commitHash: string): Promise<Tab[]> {
