...
import {AccountMessage, WelcomeMessage, WelcomeStatusMessage} from 'src/models/messages';
...
const WelcomeWrapper: React.FC = () => {
    ...
    const handleWelcome = async (info: {password: string; devMode: boolean}) => {
        await chrome.runtime.sendMessage({action: "welcome", password: info.password, devMode: info.devMode} satisfies WelcomeMessage);
        navigate("/login");
    };
...
const AuthGuardian: React.FC = () => {
    ...
    useEffect(() => {
        const checkAuthStatus = async () => {
            ...
            try {
                const isWelcomed = await chrome.runtime.sendMessage({action: "checkWelcomeStatus"} satisfies WelcomeStatusMessage);
                const isLoggedIn = await chrome.runtime.sendMessage({action: "loggedIn"} satisfies AccountMessage);
